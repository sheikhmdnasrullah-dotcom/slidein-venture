...
No semantic/vector search — `app/api/knowledge/search` is Postgres
`ilike` + array-contains on `tags` (see SECURITY.md for why that route
needed input escaping). Full-text search vector (`search_vector`, tsvector)
is generated by Postgres from indexed columns, per
`knowledge/system/workspace-app-stack.md`, but nothing does semantic ranking
on top of it.
